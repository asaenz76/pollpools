-- 0077_template_competitor_visuals — templates may optionally seed competitor
-- colors + identifier (CV.5). Recreated identically to app.provision_tenant_from_template
-- except the competitor seed loop, which now accepts BOTH a plain name string AND
-- an object { name, colors[], identifier } — string templates remain valid.

CREATE OR REPLACE FUNCTION app.provision_tenant_from_template(p_actor uuid, p_template_version_id uuid, p_identity jsonb, p_overrides jsonb, p_include_demo boolean, p_demo_owner uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  ...
  v_comp jsonb; v_cname text; v_ccolors text[]; v_cident text;
  v_snapshot jsonb;
begin
  ...
  if p_include_demo and v_seed is not null then
    ...
    -- Competitor seed accepts a plain name string OR { name, colors[], identifier }.
    for v_comp in select value from jsonb_array_elements(coalesce(v_seed->'competitors', '[]'::jsonb)) loop
      if jsonb_typeof(v_comp) = 'string' then
        v_cname := v_comp #>> '{}'; v_ccolors := '{}'::text[]; v_cident := null;
      else
        v_cname := v_comp->>'name';
        v_ccolors := coalesce((select array_agg(lower(x)) from jsonb_array_elements_text(v_comp->'colors') x), '{}'::text[]);
        v_cident := nullif(btrim(coalesce(v_comp->>'identifier', '')), '');
      end if;
      insert into competitors (tenant_id, creator_id, name, slug, visual_colors, color, identifier)
      values (v_tenant, v_creator, v_cname, 'demo-' || v_order || '-' || substr(gen_random_uuid()::text, 1, 8), v_ccolors, v_ccolors[1], v_cident)
