-- ============================================================================
-- 0023_event_youtube_required.sql  (Phase 6 refinement)
-- The Live YouTube URL is required when a creator manually creates an event, and
-- once set it may be edited but never cleared (data-integrity guard). Auto-
-- generated bracket matchup events remain nullable (the creator sets a URL later).
-- ============================================================================

-- Re-create create_event_with_market to require the Live YouTube URL.
...
begin
  ...
  if p_youtube_url is null or btrim(p_youtube_url) = '' then
    raise exception 'YOUTUBE_REQUIRED' using errcode = '22000';
  end if;
  ...
  values (v_tenant, p_competition_id, p_creator_id, p_title, p_slug, p_description, p_starts_at, p_locks_at, v_event_status, btrim(p_youtube_url), 'creator_manual')
...
-- The Live YouTube URL is editable but not deletable: once set, block clearing it.
create or replace function app.protect_event_youtube_url()
returns trigger language plpgsql as $$
begin
  if old.youtube_url is not null and btrim(old.youtube_url) <> ''
     and (new.youtube_url is null or btrim(new.youtube_url) = '') then
    raise exception 'YOUTUBE_URL_REQUIRED: the Live YouTube URL can be changed but not removed';
  end if;
  return new;
end; $$;

create trigger trg_protect_event_youtube_url
  before update on events
  for each row execute function app.protect_event_youtube_url();
